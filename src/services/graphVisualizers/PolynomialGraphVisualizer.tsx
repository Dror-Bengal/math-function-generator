...
export class PolynomialGraphVisualizer extends BaseGraphVisualizer {
  plot(
    ...
    if (typeof func !== 'function') {
      throw new Error('Polynomial visualizer requires a function');
    }
    ...
    if (IS_DEBUG) {
      ...
      if ('points' in characteristics) {
        ...
        if (Array.isArray(points)) {
          expression = 'f(x) = ' + points.map((coeff, i) => {
            const power = points.length - i - 1;
            if (coeff === 0) return '';
      ...
      DebugLogger.logGraph({
        ...mergedConfig,
        functionType: FunctionType.POLYNOMIAL,
    ...
    // Add critical points (local min/max, inflection points)
